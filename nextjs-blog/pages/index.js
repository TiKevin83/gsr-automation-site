...
export async function getStaticProps(context) {
  ...
  const files = fs.readdirSync("public/images", { withFileTypes: true });
  const verificationImages = files.map(file => {
    if (file.isFile()) {
      return file.name;
    }
  });
...
export default function Home({ verificationImages }) {
  ...
  return (
    <div className="container">
      ...
      <main>
        ...
        <p>
          Even further, the TASBot team and the rest of those involved in console verification require emulators that can replicate the exact state of the console
          at least at the time of each polled input. To learn more about console verification, you can visit the TASBot archive at <a href="https://runs.tas.bot/runs">runs.tas.bot</a>.
          The TASBot team frequently runs into issues where TASes that were previously known to sync between emulator and console stop syncing on newer revisions of the emulator.
          Many systems also aren't researched enough to have a suite of testROMs by which to evaluate their accuracy.
        </p>
        <p>
          This site aims to document a supplemental approach to testROMs for the emulator development scene, using known verified TASes as a test suite for emulation accuracy.
          Game Boy Console Verification is mainly performed using extrems' <a href="https://www.gc-forever.com/wiki/index.php?title=Game_Boy_Interface">Game Boy Interface</a>.
          Game Boy Interface takes an input log on a gamecube, with inputs timestamped relative to GBA audio samples, and sends those inputs to the Game Boy Player.
          You can find the available input logs at <a href="https://runs.tas.bot/runs">runs.tas.bot</a>, with more about how the dumps were made under Console Guides - GBC and in <a href="https://pastebin.com/ShLmxM6L">this pastebin</a>.
          The Game Boy Player attachment for the GameCube is also the main device used to record RTA speedruns of GB/GBC/GBA games.
          Therefore to maintain console accuracy for the purposes of the TASing scene, an emulator needs to be able to match the behavior of the GBA's Game Boy Color mode,
          as opposed to the behavior of the original "brick" DMG or the original Game Boy Color (the main difference being a BIOS change in the GBA-GBC mode).
          This mode was recently detailed in a <a href="https://youtu.be/_Qqg4VCZ0k8">video from Modern Vintage Gamer</a>.
        </p>
        <p>
        ...
        <p>
          ...
          Three other failures are coming from Oddworld Adventures II, Operation C, and Winnie The Pooh, Adventures in the 100 Acre Wood. Winnie the Pooh has a known syncing movie from Gambatte, but something about the dump from the published TAS isn't lining up.
